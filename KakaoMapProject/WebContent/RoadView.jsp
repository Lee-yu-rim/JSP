...
<script type="text/javascript" src="//dapi.kakao.com/v2/maps/sdk.js?appkey=56efd6c06fb480c58b736f058c3d17a4"></script>
<script>

let url = "https://api.odcloud.kr/api/15096996/v1/uddi:6738a90c-ec96-4245-a187-9528cea62904?page=1&perPage=10&serviceKey=xOwLenJh%2FEDaAUESZ%2FlRQrXL3DMw08JeMrMaaPzBVbLqHcQ7RpKA65wsL8z6fpYxBB33BWZdlE7YhtIAx5IMXg%3D%3D";
...
$.getJSON(url,function(data){
	...
	for(let i=0;i<data.currentCount;i++){
		...
		mapCenter = new kakao.maps.LatLng(data.data[i].위도, data.data[i].경도), // 지도의 중심좌표
		mapOption = {
		...
		content += '    <a href="#"><strong>' + data.data[i].오름명 + '</strong></a>';
		content += '    <div class="desc">';
		content += '        <img src="https://t1.daumcdn.net/localimg/localimages/07/mapapidoc/place_thumb.png" alt="">';
		content += '        <span class="address">' + data.data[i].위치 + '</span>';
		content += '    </div>';
		content += '</div>';
		
		// 커스텀 오버레이가 표시될 위치입니다 
		var position = new kakao.maps.LatLng(data.data[i].위도, data.data[i].경도);
